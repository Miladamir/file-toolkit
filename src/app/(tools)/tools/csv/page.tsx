...
import CSVPreview from "@/components/ui/CSVPreview";
import CSVActions from "@/components/tools/CSVActions";
...
export default function CsvToolPage() {
        ...
        <div className="relative w-full h-full" onDragEnter={handleDragEnter} onDragOver={(e) => e.preventDefault()} onDragLeave={handleDragLeave} onDrop={handleDrop}>
            {(showDropZone || isDragging) && (
                <div className={`drop-zone-container ${!showDropZone && !isDragging ? 'hidden' : ''}`}>
                    ...
                    <input ref={fileInputRef} type="file" className="hidden" accept=".csv" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
                </div>
            )}
            <CodeMirrorEditor language="csv" />
        </div>
    );

    const SeoContent = (
        <>
            <h2 className="text-2xl font-bold text-[var(--fg)] mb-4 border-b border-[var(--border)] pb-2">Online CSV Editor & Viewer</h2>
            <div className="grid md:grid-cols-2 gap-8 mt-8">
                <div>
                    <h3 className="text-lg font-semibold text-[var(--fg)] mb-2">Spreadsheet View</h3>
                    <p className="text-[var(--fg-secondary)]">Edit your CSV data in a user-friendly grid interface. Changes are synced instantly to the source code view.</p>
                </div>
                <div>
                    <h3 className="text-lg font-semibold text-[var(--fg)] mb-2">Data Management</h3>
                    <p className="text-[var(--fg-secondary)]">Sort columns, add new rows and columns, or convert your data to JSON format with a single click.</p>
                </div>
            </div>
        </>
    );
    ...
    return (
        <ToolPageLayout
            ...
            extension="csv"
            toolId="csv"
            toolbarSlot={<CSVActions />}
            editorSlot={EditorContent}
            previewSlot={<CSVPreview />}

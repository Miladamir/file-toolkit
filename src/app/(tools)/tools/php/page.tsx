...
import PHPActions from "@/components/tools/PHPActions";
import { useFileStore } from "@/store/editorStore";
import { toast } from "sonner";

export default function PhpToolPage() {
            ...
            {(showDropZone || isDragging) && (
                <div className={`drop-zone-container ${!showDropZone && !isDragging ? 'hidden' : ''}`}>
                    <div className={`drop-zone-box ${isDragging ? 'border-[var(--accent)] bg-[var(--accent-light)]' : ''}`} onClick={() => fileInputRef.current?.click()}>
                        ...
                        <div className="drop-title">Drop PHP file here</div>
                    </div>
                    <input ref={fileInputRef} type="file" className="hidden" accept=".php" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
                </div>
            )}
            <CodeMirrorEditor language="php" />
        </div>
    );

    const SeoContent = (
        <>
            <h2 className="text-2xl font-bold text-[var(--fg)] mb-4 border-b border-[var(--border)] pb-2">Online PHP Editor</h2>
            <div className="grid md:grid-cols-2 gap-8 mt-8">
                <div>
                    <h3 className="text-lg font-semibold text-[var(--fg)] mb-2">Syntax Highlighting</h3>
                    <p className="text-[var(--fg-secondary)]">Write clean PHP code with support for both PHP and HTML syntax highlighting.</p>
                </div>
                <div>
                    <h3 className="text-lg font-semibold text-[var(--fg)] mb-2">Quick Drafting</h3>
                    <p className="text-[var(--fg-secondary)]">Draft classes, functions, and logic instantly. No server setup required.</p>
                </div>
            </div>
        </>
    );
    ...
    return (
        <ToolPageLayout
            ...
            extension="php"
            toolId="php"
            toolbarSlot={<PHPActions />}
            editorSlot={EditorContent}
            seoContent={SeoContent}
            onCopy={() => { navigator.clipboard.writeText(content); toast.success("Copied!"); }}
            onDownload={() => {
                const blob = new Blob([content], { type: "text/x-php" });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a");
                a.href = url; a.download = filename || "index.php"; a.click();
            }}
        />
    );
}
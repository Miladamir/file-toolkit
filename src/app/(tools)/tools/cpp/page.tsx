...
import CppActions from "@/components/tools/CppActions";
import { useFileStore } from "@/store/editorStore";
import { toast } from "sonner";

export default function CppToolPage() {
        ...
        <div className="relative w-full h-full" onDragEnter={handleDragEnter} onDragOver={(e) => e.preventDefault()} onDragLeave={handleDragLeave} onDrop={handleDrop}>
            {(showDropZone || isDragging) && (
                <div className={`drop-zone-container ${!showDropZone && !isDragging ? 'hidden' : ''}`}>
                    ...
                    <input ref={fileInputRef} type="file" className="hidden" accept=".cpp,.hpp,.h,.cxx" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
                </div>
            )}
    ...
    const SeoContent = (
        <>
            ...
            <div className="grid md:grid-cols-2 gap-8 mt-8">
                <div>
                    <h3 className="text-lg font-semibold text-[var(--fg)] mb-2">Modern C++ Support</h3>
                    <p className="text-[var(--fg-secondary)]">Edit C++ code with syntax highlighting supporting modern standards (C++11/14/17/20).</p>
                </div>
                <div>
                    <h3 className="text-lg font-semibold text-[var(--fg)] mb-2">STL Snippets</h3>
                    <p className="text-[var(--fg-secondary)]">Quick access to common C++ patterns like vectors, maps, and standard I/O.</p>
                </div>
            </div>
        </>
    );
    ...
    return (
        <ToolPageLayout
            ...
            onDownload={() => {
                ...
                a.href = url; a.download = filename || "main.cpp"; a.click();
            }}
        />
    );
}
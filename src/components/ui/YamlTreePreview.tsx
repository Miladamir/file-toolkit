...
// Reusing the Node component logic from JSON
const TreeNode = ({ data, name, depth = 0 }: { data: any; name?: string; depth?: number }) => {
...
export default function YamlTreePreview() {
    ...
    if (!content) {
        return (
            <div className="h-full w-full flex items-center justify-center text-[var(--fg-secondary)]">
                Paste YAML to see the tree view
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="h-full w-full flex flex-col items-center justify-center p-8 text-red-500 dark:text-red-400 text-center">
                ...
                <div className="font-bold mb-2">Invalid YAML</div>
